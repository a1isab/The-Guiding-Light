-- Fix RLS infinite recursion caused by policies querying profiles table directly
-- Replaces all EXISTS (SELECT 1 FROM profiles ...) with SECURITY DEFINER get_user_role()
-- The function bypasses RLS, breaking the recursion cycle.

-- ============================================================
-- SECTION 1: Fix migration-003-rbac.sql policies
-- ============================================================

-- 1a. Fix profiles table policies (the ROOT CAUSE of recursion)
...
-- 1c. Fix sections policies
DROP POLICY IF EXISTS "Teachers can insert sections" ON sections;
CREATE POLICY "Teachers can insert sections"
  ON sections FOR INSERT
  TO authenticated
  WITH CHECK (
    EXISTS (SELECT 1 FROM courses WHERE id = course_id AND (created_by = auth.uid() OR get_user_role() = 'admin'))
  );
...
-- 1d. Fix lessons policies
DROP POLICY IF EXISTS "Teachers can insert lessons" ON lessons;
CREATE POLICY "Teachers can insert lessons"
  ON lessons FOR INSERT
  TO authenticated
  WITH CHECK (
    EXISTS (SELECT 1 FROM sections s JOIN courses c ON c.id = s.course_id WHERE s.id = section_id AND (c.created_by = auth.uid() OR get_user_role() = 'admin'))
  );
...
-- 1e. Fix quizzes policies
DROP POLICY IF EXISTS "Teachers can insert quizzes" ON quizzes;
CREATE POLICY "Teachers can insert quizzes"
  ON quizzes FOR INSERT
  TO authenticated
  WITH CHECK (
    EXISTS (SELECT 1 FROM lessons l JOIN sections s ON s.id = l.section_id JOIN courses c ON c.id = s.course_id WHERE l.id = lesson_id AND (c.created_by = auth.uid() OR get_user_role() = 'admin'))
  );
...
-- ============================================================
-- SECTION 2: Fix migration-004-teacher-classes.sql policies
-- ============================================================

-- 2a. Admin override policies on teacher-classes tables
...
-- 2b. Storage RLS for teacher-videos bucket
...
-- ============================================================
-- SECTION 3: Fix migration-005-teacher-invites.sql policies
-- ============================================================
...
-- ============================================================
-- SECTION 4: Fix migration-006-teacher-quiz.sql policies
-- ============================================================
...
-- ============================================================
-- SECTION 5: Fix migration-007-teacher-files.sql policies
-- ============================================================

-- 5a. Table-level admin override
...
-- 5b. Storage RLS for lesson-files bucket
