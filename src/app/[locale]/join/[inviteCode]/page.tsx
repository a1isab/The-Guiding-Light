      ...
      <Card className="w-full max-w-md p-8 text-center">
        ...
        {status === "success" && (
          <div data-testid="join-success">
            ...
            <h2 className="text-h4 mb-2" style={{ color: "var(--text-primary)" }}>{t("join_success_title")}</h2>
            <p className="text-sm mb-6" style={{ color: "var(--text-secondary)" }}>{t("join_success_msg", { name: className })}</p>
            <Button onClick={() => router.push(`/${params.locale}/dashboard`)} testId="join-go-to-dashboard">
              {t("go_to_dashboard")}
            </Button>
          </div>
        )}
        ...
        {status === "exists" && (
          <div data-testid="join-exists">
            ...
            <h2 className="text-h4 mb-2" style={{ color: "var(--text-primary)" }}>{t("join_exists_title")}</h2>
            <p className="text-sm mb-6" style={{ color: "var(--text-secondary)" }}>{t("join_exists_msg", { name: className })}</p>
            <Button onClick={() => router.push(`/${params.locale}/dashboard`)} testId="join-go-to-dashboard-exists">
              {t("go_to_dashboard")}
            </Button>
          </div>
        )}
        ...
        {status === "expired" && (
          <div data-testid="join-expired">
            ...
            <h2 className="text-h4 mb-2" style={{ color: "var(--text-primary)" }}>{t("join_expired_title")}</h2>
        ...
        {status === "error" && (
          <div data-testid="join-error">
            <XCircle className="h-10 w-10 mx-auto mb-4" style={{ color: "var(--error)" }} />
            <h2 className="text-h4 mb-2" style={{ color: "var(--text-primary)" }}>{t("join_error_title")}</h2>
